...
const PetProductsPage = () => {
  ...
  const fetchCarouselImages = async () => {
    try {
      const response = await axios.get("https://pawconnect1.onrender.com/Crousel/Get");
      if (Array.isArray(response.data.data)) {
  ...
  const Get = async () => {
    try {
      ...
      const data = await axios.get(`https://pawconnect1.onrender.com/Cart/Get?emailId=${emailId}`);
  ...
  const apiUrl = "https://pawconnect1.onrender.com/Cart/Add";
        ...
        >
          {sliderImages.map((image, index) => (
            <SwiperSlide key={index}>
              <div
                className="w-full h-84 bg-cover bg-no-repeat bg-center rounded-lg"
        ...
        {searchFilteredProducts.length > 0 ? (
          searchFilteredProducts.map((product) => (
            <div key={product._id} className="bg-white border border-gray-300 rounded-lg shadow-lg p-4">
              <img src={product.image} alt={product.name} className="w-full h-50 object-cover rounded-lg" />
