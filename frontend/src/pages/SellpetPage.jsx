...
export default function PetListingForm() {
 const SellPetSchema = z.object({
  petName: z.string().min(2, "Name is too short").regex(/^[A-Za-z\s]+$/, "Name must contain only letters and spaces"),
  petType: z.enum(["dog", "cat", "bird", "other"], "Select a valid pet type"),
  breed: z.string().min(2, "Breed name is too short").regex(/^[A-Za-z\s]+$/, "Breed must contain only letters and spaces").optional().or(z.literal('')),
  age: z.string().min(1, "Age is required"), // keep string for now; can parse number later if needed
  ...
  sellerInfo: z.object({
    name: z.string().min(2, "Seller name too short"),
    phone: z.string().min(7, "Phone number is too short"), 
    email: z.string().email("Invalid email address"),
    location: z.string().min(2, "Location is too short")
  }),
  // image can be validated separately if needed
});
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    SellPetSchema.parse(formData);

    // If valid, prepare FormData for upload
    const submitData = new FormData();

    if (image) {
      submitData.append('image', image);
    }
    // Append all fields manually to submitData
    ...
    submitData.append('healthvaccinated', formData.health.vaccinated.toString());
    submitData.append('healthdewormed', formData.health.dewormed.toString());
    submitData.append('healthneutered', formData.health.neutered.toString());
