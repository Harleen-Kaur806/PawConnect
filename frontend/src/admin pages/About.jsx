...
function AboutPage() {
    ...
    useEffect(() => {
        const Getdata = async () => {
            try {
                const response = await axios.get("https://pawconnect1.onrender.com/About/Get");
                if (response) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let response;
            if (editingId) {
                response = await axios.put(`https://pawconnect1.onrender.com/About/Update/${editingId}`, newData, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            } else {
                response = await axios.post("https://pawconnect1.onrender.com/About/Save", newData, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            }
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDelete) {
            try {
                await axios.delete(`https://pawconnect1.onrender.com/About/Delete/${id}`);
