...
function Services() {
    ...
    useEffect(() => {
        const Getdata = async () => {
            try {
                const response = await axios.get("https://pawconnect1.onrender.com/Service/Get");
                if (response) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let response;
            if (editingId) {
                response = await axios.put(`https://pawconnect1.onrender.com/Service/Update/${editingId}`, newData, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            } else {
                response = await axios.post("https://pawconnect1.onrender.com/Service/Save", newData, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            }
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDelete) {
            try {
                await axios.delete(`https://pawconnect1.onrender.com/Service/Delete/${id}`);
