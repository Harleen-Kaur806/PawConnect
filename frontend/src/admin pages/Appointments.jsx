...
const AppointmentPage = () => {
  ...
  const fetchAppointments = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://pawconnect1.onrender.com/Appointment/Get');
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    try {
      const response = await axios.put(`https://pawconnect1.onrender.com/Appointment/postStatus/${id}`, {
        status: newStatus
      });
